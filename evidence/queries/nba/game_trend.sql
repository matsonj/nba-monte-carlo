...
FROM src_nba_results_log )
SELECT 
    COALESCE(AR.game_date,'2025-10-20')::date AS date,
    RL.team, 
    RL.elo_rating, 
    RL.elo_change,
    sum(RL.elo_change) over (partition by team order by COALESCE(AR.game_date,'2025-10-20') asc rows between unbounded preceding and current row) as cumulative_elo_change_num0,
