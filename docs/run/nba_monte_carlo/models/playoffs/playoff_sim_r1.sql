  ...
  create view "main"."playoff_sim_r1__dbt_tmp" as (
...
WITH cte_step_1 AS (
   ...
   LEFT JOIN '/tmp/storage/random_num_gen.parquet' R ON R.game_id = S.game_id
      ...
      LEFT JOIN '/tmp/storage/initialize_seeding.parquet' EH ON S.home_team = EH.seed AND R.scenario_id = EH.scenario_id
      LEFT JOIN '/tmp/storage/initialize_seeding.parquet' EV ON S.visiting_team = EV.seed AND R.scenario_id = EV.scenario_id
