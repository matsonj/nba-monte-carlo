...
@model(
    ...
    columns={
       "game_id": "int", 
       "visiting_team": "text", 
       "visiting_team_elo_rating": "float", 
       "home_team": "text", 
       "home_team_elo_rating": "float", 
       "winning_team": "text", 
       "elo_change": "float"
    },
)
...
def execute(
    ...
    **kwargs: t.Any,
    ...
    home_adv = 100.0
    team_ratings_table = context.table("nba.raw_team_ratings")
    team_ratings = context.fetchdf(f"SELECT * FROM {team_ratings_table}")
    ...
    # add dependcy for latest results
    elo_latest_table = context.table("nba.latest_results")
    nba_elo_latest = context.fetchdf(f"SELECT * FROM {elo_latest_table}")

    # Select the specific columns
    nba_elo_latest = nba_elo_latest[["game_id", "visiting_team", "home_team", "winning_team", "margin", "game_result"]]

    # Order by 'game_id'
    nba_elo_latest = nba_elo_latest.sort_values("game_id")
    ...
    rows = []
    for index, row in nba_elo_latest.iterrows():
        game_id = row['game_id']
