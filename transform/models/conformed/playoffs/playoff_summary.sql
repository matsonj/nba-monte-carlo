...
WITH cte_playoffs_r1 AS (
    ...
    FROM {{ "'s3://datalake/conformed/initialize_seeding.parquet'" if target.name == 'parquet'
...
cte_playoffs_r2 AS (
    ...
    FROM {{ "'s3://datalake/conformed/playoff_sim_r1.parquet'" if target.name == 'parquet'
...
cte_playoffs_r3 AS (
    ...
    FROM {{ "'s3://datalake/conformed/playoff_sim_r2.parquet'" if target.name == 'parquet'
...
cte_playoffs_r4 AS (
    ...
    FROM {{ "'s3://datalake/conformed/playoff_sim_r3.parquet'" if target.name == 'parquet'
...
cte_playoffs_finals AS (
    SELECT 
        winning_team,
        COUNT(*) AS won_finals
    FROM {{ "'s3://datalake/conformed/playoff_sim_r4.parquet'" if target.name == 'parquet'
